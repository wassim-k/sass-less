...
export async function compileScssVariables(lessPath: string, callback?: (error: Less.RenderError | undefined, output: Less.RenderOutput | undefined) => void): Promise<string> {
    const variables = await extractLessVariables(lessPath, callback);
    return Object.entries(variables)
        .map(([name, value_1]) => `$${name}: ${value_1};`)
        .join('\n');
};
...
export async function extractLessVariables(
    ...
    const compileOptions: Less.Options = { filename: lessPath, javascriptEnabled: true };
